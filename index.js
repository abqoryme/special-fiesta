    ...
    if (settings.maintenance && settings.maintenance.enabled && !shouldSkip) {
      if (req.path.startsWith("/api/") || req.path.startsWith("/ai/")) {
        return res.status(503).json({
          ...
          maintenance: true,
          creator: settings.apiSettings?.creator || "4Bq0r1",
        })
      }
...
app.get("/api/preview-image", (req, res) => {
  try {
    const previewImagePath = path.join(__dirname, "src", "images", "preview.jpg")
    ...
    if (fs.existsSync(previewImagePath)) {
      ...
      const bannerPath = path.join(__dirname, "src", "images", "banner.png")
